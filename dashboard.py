...
def avg_bike_usage_by_weather(hour_df, weather_mapping):
    avg_bike_weather = hour_df.groupby('season')['cnt'].mean().reset_index()
    avg_bike_weather['weather_condition'] = avg_bike_weather['season'].map(weather_mapping)
...
day_dataframe['season'] = day_dataframe['season'].map(season_mapping)
hour_dataframe['season'] = hour_dataframe['season'].map(season_mapping)
...
with st.sidebar:
    # Menambahkan logo
    st.image("https://raw.githubusercontent.com/nadiraptrb/submission2/main/sepedaaa.png")
...
avg_bike_weather = avg_bike_usage_by_weather(main_df, weather_mapping={1: 'Spring', 2: 'Summer', 3: 'Fall', 4: 'Winter'})
...
st.caption('Copyright © 2024 Nadiraptrb. All rights reserved.')